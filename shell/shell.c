...
static int send_command(const char *command, char *reply, int reply_len, int print_errors){
	...
	snprintf(path,sizeof(path)-1,"main-%i",getuid());

	if ((pp=ortp_client_pipe_connect(path))==ORTP_PIPE_INVALID){
		if (print_errors) fprintf(stdout,"ERROR: Failed to connect pipe: %s\n",strerror(errno));
		return -1;
	}
	if (ortp_pipe_write(pp,(uint8_t*)command,strlen(command))==-1){
		if (print_errors) fprintf(stdout,"ERROR: Fail to send command to remote main\n");
...
static void print_usage(void){
	fprintf(stdout,"Usage:\nmainsh <action> [arguments]\n"
			"where action is one of\n"
			"\tinit\t\t: spawn a main daemon (first step to make other actions)\n"
			"\t\t\tfollowed by the arguments sent to main\n"
			"\tgeneric\t\t: sends a generic command to the running main daemon\n"
			...
			"\texit\t\t: make the main daemon to exit.\n"
	);
	exit(-1);
}
...
static void spawn_main(int argc, char *argv[]){
	char * args[10];
	int i,j;
	pid_t pid;
	j=0;
	...
	if (pid == 0) 
	{
		...
		printf("we are the new process\n");
		setsid();	//create a new session for a process
		printf("create a new session for a process\n");
		
		#if 1
		fd = open("/dev/null", O_RDWR);
		if (fd==-1){
			fprintf(stdout,"Could not open /dev/null\n");
			exit(-1);
		}		
		dup2(fd, 0);
		dup2(fd, 1);
		...
		if (execvp("./main",args)==-1)
		{
			fprintf(stdout,"Fail to spawn main: %s\n",strerror(errno));
			exit(-1);
		}
	}
	else if(pid>0)
...
static int register_execute(int argc, char *argv[]){
	...
	if (username==NULL) {
		fprintf(stdout,"Missing --username\n");
		print_usage();
	}
	if (host==NULL) {
		fprintf(stdout,"Missing --host\n");
		print_usage();
	}
...
static int parse_card_index(const char *reply){
	...
	if (!reply || sscanf(reply,"device #%i",&index)!=1){
		fprintf(stdout,"Error while parsing main daemon output !\n");
	}
	return index;
}
	...
	for(argi=1;argi<argc;++argi){
		if (strcmp(argv[argi],"init")==0){
			...
			if (send_generic_command("help",0)==0){
				fprintf(stdout,"A running main has been found, not spawning a second one.\n");
				return 0;
			}
			spawn_main(argc-argi-1,&argv[argi+1]);			
			return 0;
